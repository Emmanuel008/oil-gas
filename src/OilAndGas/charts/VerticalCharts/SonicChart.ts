...
export const drawSonicChart = async (rootElement: string | HTMLDivElement) => {
    ...
    const data = await getParsedData("Sonic.csv");
    ...
    const bandSeries = new FastBandRenderableSeries(wasmContext, {
        dataSeries,
        strokeThickness: 0,
        stroke: appTheme.SonicStroke,
        strokeY1: appTheme.SonicStroke,
        fill: appTheme.SonicFill,
        fillY1: appTheme.SonicFill,
    });
...
const generateSonicLegend = (
    ...
    <div class="chart-legend">
        <div class="legend-color-item" style="height: 30px;">
            <div class="color-label" style="background-color: ${appTheme.SonicStroke};"></div>
        </div>
